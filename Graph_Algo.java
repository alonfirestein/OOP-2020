...
public class Graph_Algo implements graph_algorithms {
    ...
    public graph copy() {
        ...
        graph NewGraphCopy = new Graph_DS(graph_algo);
        HashMap<Integer, node_data> NewNodesMap = new HashMap<>();
        for (node_data OriginalNode : graph_algo.getV()) {
            if (!NewGraphCopy.getV().contains(OriginalNode)) {
                node_data NewNode = NewNodesMap.get(OriginalNode.getKey());
            ...
            for (node_data neighbour : OriginalNode.getNi()) {
                if (NewGraphCopy.getV().contains(neighbour)) {
                    node_data NewNeighbour = NewNodesMap.get(neighbour.getKey());
                ...
                NewGraphCopy.connect(OriginalNode.getKey(), neighbour.getKey());
            }
        }
        return NewGraphCopy;
    }
